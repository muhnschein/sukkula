//! Every nonspacing (Mn) and enclosing (Me) mark in Unicode 16.0, as
//! inclusive code point ranges, sorted and non-overlapping.
//!
//! S2 caps how many of these may follow one base character. A table rather
//! than a handful of blocks, because stacking is not a Latin speciality: Thai
//! tone marks, Hebrew cantillation and Arabic harakat pile up exactly as
//! U+0300..U+036F does, and a cap that skips them is no cap.
//!
//! Generated from the `GeneralCategory` table of the `unicode-properties`
//! crate (UCD 16.0.0), merging adjacent Mn and Me ranges. Regenerate the same
//! way on a Unicode update; `tests::marks_table_is_sorted` checks the shape.

/// Mn and Me ranges.
#[rustfmt::skip]
pub(super) const MARKS: &[(u32, u32)] = &[
