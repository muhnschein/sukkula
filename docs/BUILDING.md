...
job, minus the two that need the network rather than this tree: `make
deny` (the RustSec advisory database) and `make vendor` (open-quickshare
at the pinned commit). A missing tool fails it, as CI's strict modes do: a
green that skipped the QML or the cross build is a green that means
nothing.

| `make` | CI job | What it gates |
|---|---|---|
| `fmt-check lint test doc` | `test` | rustfmt; clippy over the workspace and its tests, warnings denied; every test; rustdoc with broken links as errors. `--locked` throughout |
...
network; nothing after that is, until `deny` or `vendor`.
...
The image's tag carries a hash of `ci/build-sdk-image.sh` and the spec's
BuildRequires (`scripts/build-rpm.sh image`), so a pull request that
changes either derives an image of its own and cannot replace the one
`main` builds with; pull-request runs never push one.
...
which derives the image if it has to, lifts the toolchain into
`~/.cache/sukkula-sdk/`, cross-builds the engine, runs mb2, and judges the
...
everywhere but the two places that write (the SDK image, the release), and
...
`Version:`, is cut on `main`, and is published by a job of its own after
the package passed the validator.
...
- **`ci/check-deps.sh`**: the same budget over `Cargo.lock` and over
  ...
  magic-wormhole's smol pieces; Bluetooth stacks and process-spawning
  crates only with an entry in `ci/deps-allow.conf`, whose `dev` scope is
  proved absent from what ships.
