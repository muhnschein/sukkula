...
and from a clean checkout with `make check`, except where a row says it runs
on `main`: the fuzzing (every night, `fuzz.yml`) and Jolla's validator on the
built RPM (`rpm.yml`). When a row changes, change the code or test it names.
