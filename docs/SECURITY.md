...
- **The LAN is private.** LocalSend and Quick Share drop a connection from
  ...
  a global limit on offers behind it. Quick Share's mDNS reads no packet
  from outside the local link or the reach policy -- so it answers no
...
| F-QS1/F-QS4: a visible receiver is announced, and a refused announcement shows | `quickshare_mdns::receiving_announces_the_phone_and_a_second_daemon_resolves_it` (a real registration, on loopback and a port of the test's own, resolved by another mDNS daemon to the listener; `start_receiving` fails when mdns-sd refuses it); rqs_lib's `tests/mdns.rs` `the_announcement_registers_and_another_daemon_resolves_it` |
...
| Dependencies | `cargo deny` (licences, advisories, sources, bans including a vendored libdbus); `ci/check-deps.sh` (no OpenSSL, no second TLS or D-Bus stack, no process spawning); `ci/check-lockfile.sh`; `ci/vendor-check.sh` (the vendored Quick Share library and the mdns-sd under it are upstream -- open-quickshare at its commit, mdns-sd's published archive by its sha256 -- plus their reviewed patches, byte for byte) |
...
- **Upstream code is patched or guarded, not trusted.** Where a protocol
  ...
  open-quickshare's file handling (Q1–Q7) and the mDNS library under it
  (M1–M8) -- Sukkula either drives the
